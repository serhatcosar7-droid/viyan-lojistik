...
export function ViyanWebsite({ page = "home" }: { page?: PageKey }) {
...
function InnerHero({ page, t }: { page: Exclude<PageKey, "home">; t: Record<string, string | string[] | string[][]> }) {
  const content = {
...
function HomeGateway({ t }: { t: Record<string, string | string[] | string[][]> }) {
  const items = [
  ...
  return (
    <section className="section gateway-section">
      ...
      <div className="gateway-grid">
        {items.map((item) => (
          <Link className="gateway-card" href={item.href} key={item.href}>
            <h3>{item.title}</h3>
            <p>{item.text}</p>
...
function AboutSection({ t }: { t: Record<string, string | string[] | string[][]> }) {
  return (
        ...
        {filteredVehicles.map((vehicle) => (
          <article className="vehicle-card" key={vehicle.id}>
            ...
            <div className="vehicle-body">
              ...
              <div className="card-actions">
                <Link href="/iletisim" className="button compact dark">
                  {t.details as string}
                </Link>
                <a
...
function ServicesSection({ t }: { t: Record<string, string | string[] | string[][]> }) {
  return (
...
function WhySection({ t }: { t: Record<string, string | string[] | string[][]> }) {
  return (
...
function ContactSection({ t }: { t: Record<string, string | string[] | string[][]> }) {
  return (
